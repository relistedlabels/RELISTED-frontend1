...
function transformDisputeData(dispute: Dispute): ResolvedDisputeData {
  ...
  const dateCreatedStr = dateCreatedRaw
    ...
    : "—";
  const dateResolvedRaw =
    (dispute as any).dateResolved ?? (dispute as any).resolvedAt;
  const dateResolvedStr = dateResolvedRaw
    ? new Date(dateResolvedRaw).toLocaleDateString("en-US", {
        month: "short",
        day: "numeric",
        year: "numeric",
      })
  ...
  const resolution = String(
    (dispute as any).resolutionDetails ??
      (dispute as any).resolution ??
      (dispute as any).resolutionText ??
...
export default function ResolvedTable({
  ...
  const disputeColumns = buildResolvedDisputeColumns(
    setSelectedDisputeId,
    (item) => (
      <span
        className={`${adminDisputeStatusBadgeClasses(item.statusRaw)} rounded-full px-3 py-1 text-xs font-medium`}
      >
        {formatAdminDisputeStatusLabel(item.statusRaw)}
      </span>
    ),
