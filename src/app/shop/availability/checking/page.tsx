  ...
  return (
    <div className="min-h-[70vh] flex flex-col items-center justify-center px-6 py-16 text-center">
      ...
      <Paragraph1 className="max-w-md text-gray-600 leading-relaxed">
        {isAvailable &&
          (isPurchase
            ? `Great news. ${productName} is available to buy. Check your email for a one-tap sign-in link to complete checkout.`
            : `Great news. ${productName} is available for your dates. Check your email for a one-tap sign-in link to complete checkout.`)}
        {isDatesPassed &&
          (isPurchase
        ...
        {isAwaitingLister &&
          (isPurchase
            ? `We have not heard back yet on ${productName}. The lister can still approve while your request is valid. We will email you if it is approved.`
            : `We have not heard back yet on your dates for ${productName}. The lister can still approve while your dates are valid. We will email you if it is approved.`)}
        {!isAvailable &&
          !isUnavailable &&
          !isAwaitingLister &&
          (isPurchase
        ...
        <div className="mt-6 w-full max-w-sm space-y-3">
          ...
          >
            {requestMagicLink.isPending
              ? "Sending link…"
              : "Email me the login link again"}
          </button>
          {linkSent && (
            <Paragraph1 className="text-sm text-green-600">
