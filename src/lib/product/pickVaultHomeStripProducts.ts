...
/**
 * Picks up to `limit` products for the home sale strip by round-robin across
...
export function pickVaultHomeStripProducts(
  ...
  const withoutCloset: UserProduct[] = [];
  for (const p of products) {
    const cid = p.closet?.id ?? p.closetId;
    if (!cid) {
      withoutCloset.push(p);
      continue;
    }
